...
if [ -n "$GOPAY_SESSION_JSON" ]; then
    echo "$GOPAY_SESSION_JSON" > /home/user/app/gopay-gateway/.GOPAY_SESI_JANGAN_DIHAPUS.json
    echo "🔑 [GOPAY] Sesi GoBiz berhasil dimuat dari environment secrets."
fi

# 2. Buat file gopay-gateway/.env jika belum ada
if [ ! -f /home/user/app/gopay-gateway/.env ]; then
    cat <<EOF > /home/user/app/gopay-gateway/.env
PORT=3005
API_KEY=RAHASIA
...
echo "🟢 [PM2] Menjalankan gopay-gateway, p2p-telegram-bot, dan status-monitor..."
